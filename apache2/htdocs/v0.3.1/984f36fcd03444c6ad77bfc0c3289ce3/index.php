...
foreach ($mailNotif->notifySMP() as $key => $issue) {
   ...
   if ($datediff <= 5 && $datediff >= -5) {
      $isNotified = $mailNotif->isNotified("issue_id = ?", [$issue->id]);
      if ($isNotified['isNotified']) {
         $issue->isNotified = true;

         $datediff = strtotime($isNotified['created_at']) - $date;
         $datediff = round($datediff / (60 * 60 * 24));

         if ($datediff == 2) {
            $issue->isNotified = false;
         } else {
            $mailNotif->updatedNotifys([$issue->id]);
         }
      } else {
         $issue->isNotified = false;

         $table = 'notification_';
         $columns = "issue_id, user_id, emailed_to, is_notified, n_nots, created_at, updated_at";
         $fields = "?, ?, ?, ?, ?, now(), now()";
         $values = [$issue->id, $issue->author_id, $issue->email_to, true, 1];

         $saveNotify = $mailNotif->storeNotification($table, $columns, $fields, $values);
      }
      if (!$issue->isNotified) {
...
foreach ($issues as $key => $issue) {
   ...
   // echo $message;
   sendMain($message, $email_to, $name);
