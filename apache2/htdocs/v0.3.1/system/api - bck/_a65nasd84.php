...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			case 'actividadePROJ':
				...
				if(isset($_GET['ano']) && !isset($_GET['Px']) && !isset($_GET['Py'])){
					...
					$ano = $_GET['ano'];
					$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.subject AS indicador, prod.subject AS 	produto, resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
							COALESCE(case when cv.value = null or cv.value = '' then '0' else cv.value end) AS metaNumerica, mD.value AS metaDescritiva,
							obs.value AS observacoes, fv.value AS fonteVerificacao, COALESCE(case when rea.value = null or rea.value = '' then '0' else rea.value end) AS realizado, var.value AS variavel
							from issues AS i
							INNER JOIN trackers AS t ON (i.tracker_id = t.id)
							...
							LEFT JOIN custom_values AS rea ON (rea.custom_field_id=105 AND rea.customized_id=i.id)
							LEFT JOIN custom_values AS var ON (var.custom_field_id=99 AND var.customized_id=i.id)
							WHERE i.tracker_id = 11 AND i.project_id= ? and year(i.start_date) = ? Order by t.position ASC, i.root_id ASC");
					...
					$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.subject AS indicador, prod.subject AS 	produto, resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
							...
							LEFT JOIN custom_values AS rea ON (rea.custom_field_id=105 AND rea.customized_id=i.id)
							LEFT JOIN custom_values AS var ON (var.custom_field_id=99 AND var.customized_id=i.id)
							WHERE i.tracker_id = 11 AND i.project_id= ?
							and year(i.start_date) = ? and month(i.start_date) >= ? and month(i.start_date) <= ?
							Order by t.position ASC, i.root_id ASC");
				...
				$n = null;
				while ($info_report = $get_reports->fetchObject()) {
					var_dump($get_reports);
					$realizado = $info_report->realizado;
					$metaNumerica = $info_report->metaNumerica;

					if($info_report->realizado == 1){
						$realizado = 1;
					}

					if($info_report->metaNumerica == 1){
						$metaNumerica = 1;
					}
			...
			case 'OrcamentoPDE':
				...
				$get_projectos->execute(array($_GET['p']));
					...
					while ($programa = $get_projectos->fetchObject()) {
						...
						$get_orcObjectivos = $db->prepare("SELECT
			...
			case 'reportOrcamento':
				...
				if(isset($_GET['ano']) && !isset($_GET['Px']) && !isset($_GET['Py'])){
					...
					$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.subject AS indicador, prod.subject AS produto,
						...
						WHERE i.tracker_id = 11 AND i.project_id= ? AND year(i.start_date) = ? Order by t.position ASC");
					$get_reports->execute(array($id_report, $ano));
					...
					$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.subject AS indicador, prod.subject AS produto,
						...
						LEFT JOIN custom_values AS obs ON (obs.custom_field_id=51 AND obs.customized_id=ind.id)
						LEFT JOIN custom_values AS fv ON (fv.custom_field_id=46 AND fv.customized_id=ind.id)
						WHERE i.tracker_id = 11 AND i.project_id=? AND year(i.start_date) = ? and month(i.start_date) >= ? and month(i.start_date) <= ? Order by t.position ASC");
					$get_reports->execute(array($id_report, $ano, $Px, $Py));
				...
				if($get_reports->rowCount() < 1){
					$orcamento = 0.00;
					$orcamentovalorGasto = 0.00;
				}else{
					$info_report = $get_reports->fetchObject();
				...
				while ($info_report = $get_reports->fetchObject()) {
					...
					if(number_format((float)($orcamentovalorGasto)) == 0 && number_format((float)($orcamento)) != 0){
						...
						$percent = number_format((float)((($orcamentovalorGasto) / (number_format((float)($orcamento)))) * 100), 2, '.', '') ."%"; // Formatando o valor para percentual
					}
					if(number_format((float)($orcamentovalorGasto)) != 0 && number_format((float)($orcamento)) == 0){
						$percent = "0.00%";
					}
					...
					$report_result .= "<tr>
						...
						<td>".$info_report->actividade."</td>
						<td>".$info_report->indicador."</td>
						<td>".number_format((float)($orcamento), 2, '.', ',')."</td>
