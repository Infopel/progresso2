 ...
 * **/
 ...
 class reportHelper{
	...
	public function reportOrcProj_($p)
	{
		$id_report = $p;
		$A = null;
		$ds  = null;
	...
	//	 deprecaded functin
	public function reportActProj_($p)
	{
			$id_report = $p;
			$id_report = $p;
			$A = null;
	...
	public function reportActProj($p)
	{
		...
		if(isset($_GET['ano']) && !isset($_GET['Px']) && !isset($_GET['Py'])){
					...
					$ano = $_GET['ano'];
					$get_reports = $this->conn->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto, resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
		...
		$title = "Relatório - Actividades Por Projecto";
		$DOMhtml = "<table border='1'>
						<thead>
							<tr><td colspan='11'><h4 style='text-transform: uppercase'>Associação PROGRESSO</h4></td></tr>
							<tr>
	...
	public function reportOrcProj($p)
	{
		$id_report = $p;
				...
				if(isset($_GET['ano']) && !isset($_GET['Px']) && !isset($_GET['Py'])){
					...
					// Staring the Query
					$get_reports = $this->conn->prepare("SELECT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto,
						resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
