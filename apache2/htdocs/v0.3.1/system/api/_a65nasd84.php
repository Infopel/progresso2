...
global $valorGastoProjectos, $orcamentoProjectos, $orcamentoPDs, $listProgramas, $dataListvGasto, $dataListOrc, $dataListvObjesp, $dataListChart, $listaProg;
    // Switch on each action
    switch ($_GET['report']) {

			case 'filter_ano':
				$get_anos = $db->prepare('SELECT YEAR(start_date) as ano FROM issues  WHERE start_date is not null group by YEAR(start_date)');
				$get_anos->execute(array());

				$response = null;
				while($anos = $get_anos->fetchObject()){
					$response [] = array(
						"ano" => $anos->ano,
					);
				}
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			case 'actividadePROJ':
				...
				$report_result = null;
				if(empty($id_report)){
					$id_report = 17; // ID do projecto pre-indicado
				}else{
					$id_report = $_GET['p'];
				}
				...
				if(isset($_GET['ano']) && !isset($_GET['Px']) && !isset($_GET['Py'])){
					...
					$ano = $_GET['ano'];
					$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto, resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
							COALESCE(case when cv.value = null or cv.value = '' or cv.value is null then '0' else cv.value end) AS metaNumerica, fv.value AS fonteVerificacao, COALESCE(SUM(case when rea.value = null or rea.value = '' or rea.value is null then '0' else rea.value end)) AS realizado, var.value AS variavel
							from issues AS i
							INNER JOIN trackers AS t ON (i.tracker_id = t.id)
							LEFT JOIN custom_values AS ind ON (ind.custom_field_id=97 AND ind.customized_id=i.id)
                            ...
                            LEFT JOIN custom_values AS rea ON (rea.customized_type = 'Issue' AND i.id = rea.customized_id AND rea.custom_field_id = 105)							LEFT JOIN custom_values AS var ON (var.custom_field_id=99 AND var.customized_id=i.id)
							WHERE i.tracker_id = 11 AND i.project_id= ? and year(i.start_date) = ? group by i.id Order by resImed.id ASC");
					$get_reports->execute(array($id_report, $ano));
				}else if(isset($_GET['ano'], $_GET['Px'], $_GET['Py'])){
					$ano = $_GET['ano'];
					$Px = $_GET['Px'];
					$Py = $_GET['Py'];
					...
					$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto, resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
                            ...
                            LEFT JOIN custom_values AS rea ON (rea.customized_type = 'Issue' AND i.id = rea.customized_id AND rea.custom_field_id = 105)							LEFT JOIN custom_values AS var ON (var.custom_field_id=99 AND var.customized_id=i.id)
							and year(i.start_date) = ? and month(i.start_date) >= ? and month(i.start_date) <= ? group by i.id Order by resImed.id ASC");
					$get_reports->execute(array($id_report, $ano, $Px, $Py));
					...
					$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto, resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
                            ...
                            LEFT JOIN custom_values AS rea ON (rea.customized_type = 'Issue' AND i.id = rea.customized_id AND rea.custom_field_id = 105)
							LEFT JOIN custom_values AS var ON (var.custom_field_id=99 AND var.customized_id=i.id)
							WHERE i.tracker_id = 11 AND i.project_id= ? and i.start_date >= ? and i.due_date <= ? group by i.id Order by resImed.id ASC");
					$get_reports->execute(array($id_report, $startDate, $endDate));

					if($get_reports->rowCount() == 0){
						$report_result = "<tr><td colspan='10' style='text-align:center'>Nenhum resultado encontrado para projectos com inicio em: <b>$startDate</b> e Fim em: <b>$endDate</b></td></tr>";
					}
					...
					$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto, resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
                            ...
                            LEFT JOIN custom_values AS rea ON (rea.customized_type = 'Issue' AND i.id = rea.customized_id AND rea.custom_field_id = 105)							LEFT JOIN custom_values AS var ON (var.custom_field_id=99 AND var.customized_id=i.id)
							WHERE i.tracker_id = 11 AND i.project_id= ? and i.start_date >= ? group by i.id Order by resImed.id ASC");
					$get_reports->execute(array($id_report, $startDate));

					if($get_reports->rowCount() == 0){
						$report_result = "<tr><td colspan='10' style='text-align:center'>Nenhum resultado encontrado para projectos com inicio em: <b>$startDate</b></td></tr>";
					}
					...
					$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto, resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
							...
							LEFT JOIN custom_values AS cv ON (cv.custom_field_id=100 AND cv.customized_id=i.id)
							LEFT JOIN custom_values AS fv ON (fv.custom_field_id=46 AND fv.customized_id=i.id)
							LEFT JOIN time_entries AS t_entrie ON (t_entrie.issue_id = i.id)
							...
							LEFT JOIN custom_values AS var ON (var.custom_field_id=99 AND var.customized_id=i.id)
							WHERE i.tracker_id = 11 AND i.project_id= ? and i.due_date <= ? group by i.id Order by resImed.id ASC");
					$get_reports->execute(array($id_report, $endDate));

					if($get_reports->rowCount() == 0){
						$report_result = "<tr><td colspan='10' style='text-align:center'>Nenhum resultado encontrado para projectos com data de Fim em: <b>$startDate</b></td></tr>";
					}
				}else{
						$get_reports = $db->prepare("SELECT DISTINCT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto, resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
							COALESCE(case when cv.value = null or cv.value = '' or cv.value is null then '0' else cv.value end) AS metaNumerica, fv.value AS fonteVerificacao, COALESCE(SUM(case when rea.value = null or rea.value = '' or rea.value is null then '0' else rea.value end)) AS realizado, var.value AS variavel
                            ...
                            LEFT JOIN custom_values AS rea ON (rea.customized_type = 'Issue' AND i.id = rea.customized_id AND rea.custom_field_id = 105)							LEFT JOIN custom_values AS var ON (var.custom_field_id=99 AND var.customized_id=i.id)
							WHERE i.tracker_id = 11 AND i.project_id= ? group by i.id Order by resImed.id ASC");
					$get_reports->execute(array($id_report));
				}
				...
				$n = null;
				while ($info_report = $get_reports->fetchObject()) {
					$realizado = $info_report->realizado;
					$metaNumerica = $info_report->metaNumerica;

					if($info_report->realizado == 0){
						$realizado = 0;
					}

					if($info_report->metaNumerica == 0){
						$metaNumerica = 1;
					}
					$percent = number_format((float)((($realizado) / ($metaNumerica)) * 100), 2, '.', '') ."%"; // Formatando o valor para percentual
					$report_result .= "<tr>
					...
					$resultadoFinal = $info_report->resultadoFinal;
				}
				$response_table =
				"<thead>
					 <tr class='p-2 border-0' style=' font-size: 0.78rem; '>
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			case 'actividadePROV':
				...
				if(isset($_GET['p']) && !isset($_GET['ano']) && !isset($_GET['Px']) && !isset($_GET['Py'])){
					$get_projectos = $db->prepare('SELECT i.id AS id_actividade, i.subject AS actividade, projecto.name AS nome_projecto, cv_p.value AS provincia
					FROM custom_values AS cv_p
						LEFT JOIN issues AS i ON (i.id = customized_id)
						LEFT JOIN projects AS projecto ON (projecto.id = project_id)
					WHERE value = ? AND customized_type = "Issue" and status = 1 ORDER BY nome_projecto;');

					$get_projectos->execute(array($provincia));
					$report_result = null;
					while ($info = $get_projectos->fetchObject()) {

						$Indicadores = $db->prepare("SELECT i.id AS id_indicador, i.subject AS indicador, projecto.name AS nome_projecto, meta.value as meta, realizado.value as realizado
							FROM issues as i
								LEFT JOIN projects AS projecto ON (projecto.id = project_id)
								LEFT JOIN custom_values AS meta ON (meta.custom_field_id = 100 AND meta.customized_id = i.id)
								LEFT JOIN custom_values AS realizado ON (realizado.custom_field_id = 105 AND realizado.customized_id = i.id)
							WHERE i.parent_id = ? and status = 1");

						$Indicadores->execute(array($info->id_actividade));
						$getIndicador = $Indicadores->fetchObject();

						if($Indicadores->rowCount() > 0){
							$indicador = $getIndicador->indicador;
						}else{
							$indicador = null;
						}
							$get_realizadoMeta = $db->prepare("SELECT COALESCE(case when realizado.value = null then '' else realizado.value end) as realizado, COALESCE( case when meta.value = null then '' else meta.value end) as meta
								FROM custom_values as realizado
								inner join custom_values as meta on (meta.custom_field_id = 100 and meta.customized_id = ?)
								WHERE realizado.customized_id = ? and realizado.custom_field_id = 105");

							$get_realizadoMeta->execute(array($info->id_actividade, $info->id_actividade));
							$getRealizado = $get_realizadoMeta->fetchObject();

							if($get_realizadoMeta->rowCount() > 0){
								$realizado = $getRealizado->realizado;
								$meta = $getRealizado->meta;
							}else{
								$realizado = null;
								$meta = null;
							}
				...
				if(isset($_GET['ano']) && !isset($_GET['Px']) && !isset($_GET['Py'])){
					...
					FROM custom_values AS cv_p
						LEFT JOIN issues AS i ON (i.id = customized_id)
						LEFT JOIN projects AS projecto ON (projecto.id = project_id)
					WHERE value = ? AND customized_type = "Issue" and year(i.start_date) = ? and status = 1 ORDER BY nome_projecto;');

					$get_projectos->execute(array($provincia, $ano));
					$report_result = null;
					while ($info = $get_projectos->fetchObject()) {
					...
					FROM custom_values AS cv_p
						LEFT JOIN issues AS i ON (i.id = customized_id)
						LEFT JOIN projects AS projecto ON (projecto.id = project_id)
					WHERE value = ? AND customized_type = "Issue" and year(i.start_date) = ? and month(i.start_date) >= ? and month(i.start_date) <= ? and status = 1 ORDER BY nome_projecto;');

					$get_projectos->execute(array($provincia, $ano, $Px, $Py));
					$report_result = null;
					while ($info = $get_projectos->fetchObject()) {
			...
			case 'plano':
				...
				$plano = $get_planoEst->fetchObject();
				echo json_encode([
					'id' => $plano->id_proj,
			...
			case 'OrcamentoPDE':
				...
				while ($programa = $get_projectos->fetchObject()) {
					...
					// Pegar as actividades do projecto
					$getActividade = $db->prepare('SELECT id FROM projects where parent_id = ?');
					$getActividade->execute(array($programa->id));
					$id_actividade = $getActividade->fetchObject()->id;

					$getValorGasto = $db->prepare("SELECT
						...
						WHERE
							...
							AND projecto.parent_id != ' '
						");
					$getValorGasto->execute(array($programa->id));

					$valor_gasto = $getValorGasto->fetchObject()->valorGasto;
					...
					$get_orcObjectivos = $db->prepare("SELECT i.subject as obj_especifico, orc_esp.value as orc_objectivo
						FROM
					...
					$json_response[] = array(
						...
						"valor_gasto" => number_format((float)(($valor_gasto)), 2, '.', ',')
					);
					...
					$valorGastoProjectos .= number_format((float)(($valor_gasto)), 2, '.', '') .", ";
			...
			case 'OrcamentoPROJ':
				...
				// Staring the Query
					...
					$get_projectos = $db->prepare('SELECT projectos.id, projectos.name as nome_projecto, orc.value as orc_previsto
					FROM projects AS projectos
					...
					WHERE projectos.id = ? AND status = 1 and parent_id != 12
					');
			...
			case 'reportOrcamento':
				...
				if(isset($_GET['ano']) && !isset($_GET['Px']) && !isset($_GET['Py'])){
					...
					// Staring the Query
					$get_reports = $db->prepare("SELECT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto,
						resImed.subject AS resultadoImediato, resInter.subject AS resultadoIntermedio, resFinal.subject AS resultadoFinal,
						COALESCE(case when orc.value = '' or orc.value is null then '0' else orc.value end) AS orcamento,
                        COALESCE(SUM(case when vg.value = '' or vg.value  is null then '0' else vg.value end)) AS valorGasto, fv.value AS fonteVerificacao
						from issues AS i
						INNER JOIN trackers AS t ON (i.tracker_id = t.id)
						LEFT JOIN custom_values AS ind ON (ind.custom_field_id=97 AND ind.customized_id=i.id)
						LEFT JOIN issues AS prod ON (i.parent_id = prod.id AND prod.tracker_id=2)
					...
					$get_reports = $db->prepare("SELECT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto,
                        ...
                        LEFT JOIN custom_values AS vg ON (vg.customized_type = 'TimeEntry' AND t_entrie.id = vg.customized_id AND vg.custom_field_id = 119)
						LEFT JOIN custom_values AS fv ON (fv.custom_field_id=46 AND fv.customized_id=ind.id)
						WHERE i.tracker_id = 11 AND i.project_id=? AND year(i.start_date) = ? and month(i.start_date) >= ? and month(i.start_date) <= ? group by i.id Order by resImed.id ASC");
					$get_reports->execute(array($id_report, $ano, $Px, $Py));
					$report_result = "<tr><td colspan='9' style='text-align:center'>Nenhum resultado encontrado para projectos com inicio em: <b>$startDate</b></td></tr>";
				}else if(isset($_GET['startDate']) && isset($_GET['endDate'])){ // Por Data de Inicio e Data de Fim
					$startDate = date('Y-m-d', strtotime($_GET['startDate']));
					$endDate = date('Y-m-d', strtotime($_GET['endDate']));
					...
					if($get_reports->rowCount() == 0){
						$report_result = "<tr><td colspan='9' style='text-align:center'>Nenhum resultado encontrado para projectos com inicio em: <b>$startDate</b> e Fim em: <b>$endDate</b></td></tr>";
					}
				}else if(isset($_GET['startDate']) && !isset($_GET['endDate'])){ // Por Data de Inicio
					...
					if($get_reports->rowCount() == 0){
						$report_result = "<tr><td colspan='9' style='text-align:center'>Nenhum resultado encontrado para projectos com a data de fim: <b>$endDate</b></td></tr>";
					}
				}else{
					// Staring the Query
					$get_reports = $db->prepare("SELECT i.id, i.subject AS actividade, t.name AS name, ind.value AS indicador, prod.subject AS produto,
				...
				while ($info_report = $get_reports->fetchObject()) {
					...
					$orcamento = $info_report->orcamento;
					$orcamentovalorGasto = $info_report->valorGasto;

					if(number_format(($orcamentovalorGasto)) == 0 && number_format((float)($orcamento)) != 0){

						$percent = number_format((float)(($orcamentovalorGasto / $orcamento) * 100), 2, '.', '') ."%"; // Formatando o valor para percentual
					}
					if(number_format((float)($orcamentovalorGasto)) != 0 && number_format((float)($orcamento)) == 0){
						$percent = "0.00%";
					}
					...
					$report_result .= "<tr>
						...
						<td>".$info_report->actividade."</td>
						<td>".$info_report->indicador."</td>
						<td><b>".number_format((float)($orcamento), 2, '.', ',')."</b></td>
						<td><b>".number_format((float)($info_report->valorGasto), 2, '.', ',')."</b></td>
						<td><b>".$percent."<b></td>
						<td>".$info_report->fonteVerificacao."</td>
						</tr>";
				}
			...
			case "beneficiariosPDE":
				...
				$get_beneficiarios = $db->prepare("SELECT i.id, projecto.name as nome_projecto, variavel_bnf.value as variavel_bnf,
						COALESCE(SUM(case when num_bnf.value is null or num_bnf.value = '' then '0' else num_bnf.value end)) as num_bnf
					FROM
						time_entries AS te
						INNER JOIN issues AS i ON (i.id = te.issue_id)
						INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 121)
						INNER JOIN custom_values as num_bnf on (num_bnf.customized_id = te.id and num_bnf.custom_field_id = 120)
						LEFT JOIN projects as projecto on (projecto.id = i.project_id)
					...
					WHERE
						...
						and projecto.parent_id is not null
						and pde.parent_id = ?
						and variavel_bnf.value != ' '
						and num_bnf.value != ' '
						group by variavel_bnf, nome_projecto
				...
				$get_beneficiariosMateria = $db->prepare("SELECT i.id, projecto.name as nome_projecto, variavel_bnf.value as variavel,
						COALESCE(SUM(case when num_bnf.value is null or num_bnf.value = '' then '0' else num_bnf.value end)) as num_bnf
					FROM
						time_entries AS te
						INNER JOIN issues AS i ON (i.id = te.issue_id)
						INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 122)
						INNER JOIN custom_values as num_bnf on (num_bnf.customized_id = te.id and num_bnf.custom_field_id = 120)
						LEFT JOIN projects as projecto on (projecto.id = i.project_id)
						LEFT JOIN projects as pde on (pde.id = projecto.parent_id)
					WHERE
						...
						and variavel_bnf.value != ' '
						and num_bnf.value != ' '
						group by variavel, nome_projecto
						order by nome_projecto");
				$get_beneficiariosMateria->execute(array($id_report));

				$get_beneficiariosMaterias = $db->prepare("SELECT i.id, projecto.name as nome_projecto, variavel_bnf.value as variavel,
						COALESCE(SUM(case when num_bnf.value is null or num_bnf.value = '' then '0' else num_bnf.value end)) as num_bnf
					FROM
						time_entries AS te
						INNER JOIN issues AS i ON (i.id = te.issue_id)
					...
					WHERE
						...
						group by variavel, nome_projecto
						order by nome_projecto");
				$get_beneficiariosMaterias->execute(array($id_report));

				$resp_benMaterial = null;
				$resp_benMaterias = null;

				while ($beneficiariosMateria = $get_beneficiariosMateria->fetchObject()){

					$resp_benMaterial[] = array(
								"_projecto" => $beneficiariosMateria->nome_projecto,
								"_variavel" => $beneficiariosMateria->variavel,
								"_beneficarios" => $beneficiariosMateria->num_bnf
						);
				}

					$_total = null; $_variavel = null; $d = ''; $i = 1;
					$rows = $get_beneficiariosMaterias->rowCount();
					while ($beneficiariosMaterias = $get_beneficiariosMaterias->fetchObject()){
						if ($i < $rows)
						{
							$d = ', ';
						}
						...
						$_total .= "$beneficiariosMaterias->num_bnf".$d."";
						$_variavel .= "$beneficiariosMaterias->variavel".$d."";
						$resp_benMaterias= array(
									"_variavel" => $_variavel,
									"_total" => $_total
							);
						$i++;
					...
					while ($beneficiarios = $get_beneficiarios->fetchObject()) {
						...
						$infoBeneficiarios .= "<tr><td>".$beneficiarios->variavel_bnf."</td><td>".$beneficiarios->num_bnf."</td></tr>";
						$infoProjectos .= "<tr><td>".$beneficiarios->nome_projecto."</td><td>".$beneficiarios->variavel_bnf."</td><td>".$beneficiarios->num_bnf."</td></tr>";
						//Armanzena Beneficiarios por faixa etaria
						$resp_benFaixaEtaria [] = array(
									"_projecto" => $beneficiarios->nome_projecto,
									"_faixaEtaria" => $beneficiarios->variavel_bnf,
									"_beneficarios" => $beneficiarios->num_bnf
							);
						$ben_GraphFEtaria .= "'".$beneficiarios->variavel_bnf."', ";
						$ben_GraphBeneficiarios .= $beneficiarios->num_bnf.", ";

						// ['(>50) Homens e Mulheres', 2]
					}// End Loop While

					if( $get_beneficiarios->rowCount() > 0){
							// Soma todos bnf do sexo Femenino
						$get_bnfMulheres = $db->prepare("SELECT sum(num_bnf.value) as bnf_mulheres
							FROM
								time_entries AS te
								INNER JOIN issues AS i ON (i.id = te.issue_id)
								INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 121)
							...
							WHERE
								...
								and pde.parent_id = ?
								and variavel_bnf.value != ' '
								and variavel_bnf.value like '%mulher%'
								and num_bnf.value != ' '
							");
						...
						$get_bnfHomens = $db->prepare("SELECT sum(num_bnf.value) as bnf_homens
							FROM
								time_entries AS te
								INNER JOIN issues AS i ON (i.id = te.issue_id)
								INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 121)
							...
							WHERE
								...
								and pde.parent_id = ?
								and variavel_bnf.value != ' '
								and variavel_bnf.value like '%home%'
								and num_bnf.value != ' '
							");
						...
						$get_bnf_criancas = $db->prepare("SELECT sum(num_bnf.value) as bnf
							FROM
								time_entries AS te
								INNER JOIN issues AS i ON (i.id = te.issue_id)
								INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 121)
							...
							WHERE
								...
								and variavel_bnf.value != ' '
								and num_bnf.value != ' '
								and variavel_bnf.value like '%crianças%'
							");

						$get_bnf_criancas->execute(array($id_report));
						$bnf_criancas = $get_bnf_criancas->fetchObject();

						// Soma de Beneficiarios Homens de 0 a 5 anos
						$get_bnfHM_05 = $db->prepare("SELECT sum(num_bnf.value) as bnf
							FROM
								time_entries AS te
								INNER JOIN issues AS i ON (i.id = te.issue_id)
								INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 121)
							...
							WHERE
								...
								and variavel_bnf.value != ' '
								and num_bnf.value != ' '
								and variavel_bnf.value like '%0-5%'
								");
						...
						$get_bnfHM_06 = $db->prepare("SELECT sum(num_bnf.value) as bnf
							FROM
								time_entries AS te
								INNER JOIN issues AS i ON (i.id = te.issue_id)
								INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 121)
							...
							WHERE
								...
								and variavel_bnf.value != ' '
								and num_bnf.value != ' '
								and variavel_bnf.value like '%6-14%'
							");
						...
						$get_bnfHM_15 = $db->prepare("SELECT sum(num_bnf.value) as bnf
							FROM
								time_entries AS te
								INNER JOIN issues AS i ON (i.id = te.issue_id)
								INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 121)
							...
							WHERE
								...
								and variavel_bnf.value != ' '
								and num_bnf.value != ' '
								and variavel_bnf.value like '%15-24%'
							");
						...
						$get_bnfHM_25 = $db->prepare("SELECT sum(num_bnf.value) as bnf
							FROM
								time_entries AS te
								INNER JOIN issues AS i ON (i.id = te.issue_id)
								INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 121)
							...
							WHERE
								...
								and variavel_bnf.value != ' '
								and num_bnf.value != ' '
								and variavel_bnf.value like '%25-49%'
							");
						...
						// Soma de Beneficiarios Homens e Mulheres de 25 a 49 anos
						$get_bnfHM_50 = $db->prepare("SELECT sum(num_bnf.value) as bnf
							FROM
								time_entries AS te
								INNER JOIN issues AS i ON (i.id = te.issue_id)
								INNER JOIN custom_values as variavel_bnf on (variavel_bnf.customized_id = te.id and variavel_bnf.custom_field_id = 121)
							...
							WHERE
								...
								and variavel_bnf.value != ' '
								and num_bnf.value != ' '
								and variavel_bnf.value like '%>50%'
							");
			...
			case "beneficiariosPROJ":
				...
				// Staring the Query
					$get_beneficiarios = $db->prepare("SELECT
							i.id, projecto.id, projecto.name as nome_projecto, variavel_bnf.value as variavel_bnf
							,COALESCE(SUM(case when num_bnf.value is null or num_bnf.value = '' then '0' else num_bnf.value end)) as num_bnf
						FROM
							time_entries AS te
							INNER JOIN issues AS i ON (i.id = te.issue_id)
				...
				while ($beneficiarios = $get_beneficiarios->fetchObject()) {
					...
					$dataGraphic = "".($bnf_criancas->bnf).", ".($bnfHM_05->bnf).", ".($bnfHM_06->bnf).", ".($bnfHM_15->bnf).", ".($bnfHM_25->bnf).", ".($bnfHM_50->bnf)."";
