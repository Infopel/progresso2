...
include_once('v0.3.1/system/helper/issuesUpdate.php');


$db = BD::conn();

$issuesUpdate = new helperIssuesUpdate($db);

$issuesUpdate->getIssuesParents();
		...
		$get_programas = $db->prepare("SELECT distinct projecto.name AS nome_projecto, p_parent.name as programa
			FROM
			    ...
			    issues AS i ON (i.id = customized_id)
			        LEFT JOIN
			    projects AS projecto ON (projecto.id = cv.customized_id)
			    left join
		...
		if ($get_programas->rowCount() == 0) {
			...
			$get_nums = $db->prepare("SELECT
				...
				FROM
					...
					issues AS i ON (i.id = customized_id)
						LEFT JOIN
					projects AS projecto ON (projecto.id = customized_id)
					left join
					projects AS p_parent ON (p_parent.id = projecto.parent_id)
				WHERE
					value = ?
			...
			// Data Info response
			$dataListPro .= '<tr><td>' . $programa->nome_programa . '</td><td>' . $dir . '</td><td>' . $return_nums->num_projectos . '</td></tr>';
		}
	}
}
?>
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel-body">
				<div class="col-md-3" style="max-width:370px; min-width: 370px; margin-left: 20px; margin-right: 20px">
					<div>
						...
						<map name="mocambique" id="mocambique">
							...
							<a href="?prov=Maputo-Província" itemid="2">
								<area shape="poly" style="fill: red" href="?prov=Maputo-Província" rev="-327,-132,37,83,48,409" id="Maputo-Provincia" alt="Maputo-Provincia" title="Maputo-Provincia" coords="80,440, 79,437, 78,434, 78,427, 72,419, 72,416, 65,415, 61,411, 55,411, 53,413, 53,414, 54,415, 55,429, 55,439, 55,449, 55,453, 52,455, 52,462, 56,465, 58,469, 56,473, 57,480, 59,490, 61,490, 63,492, 65,491, 76,491, 77,488, 78,478, 78,475, 78,472, 79,467, 78,468,76,472, 72,468, 69,465,67,466, 65,466, 66,465, 72,461, 73,457, 76,451, 82,446, 84,445, 83,445, 82,442, 80,440">
							...
							<a href="?prov=Nampula" itemid="9">
								<area shape="poly" style="fill: red" href="?prov=Nampula" rev="-344,-9,116,107,171,87" id="Nampula" alt="Nampula" title="Nampula" coords="266,90, 263,91, 259,95, 256,95, 249,98, 247,100, 241,103, 240,103, 239,104, 226,108, 219,108, 219,107, 218,109, 212,111, 209,112, 206,113, 205,111, 202,112, 198,112, 198,114, 197,116, 195,116, 194,118, 193,119, 186,122, 183,123, 177,131, 173,135, 186,135, 189,138,193,138, 195,138, 200,142,202,140, 206,141, 212,146, 217,148, 224,155, 226,156, 231,165, 231,169, 231,170, 235,180, 233,184, 234,186, 237,191, 237,191, 253,181, 255,179, 255,176, 257,174, 259,172, 265,163, 275,152, 279,146, 279,142, 276,142, 275,141, 276,140, 280,138, 280,136, 279,134, 282,131, 284,123, 284,119, 281,119, 278,119, 278,118, 280,116,280,111, 277,113, 276,111, 278,107, 276,95, 278,94, 274,92, 276,90, 274,90, 266,90">
				...
				<div class="col-md-6" style="background:#ffffff; padding-top:15px; border: 1px solid #dedede; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.03), 0 1px 1px 0 rgba(0, 0, 0, 0.02), 0 2px 1px -1px rgba(70, 67, 67, 0.22);
    border-radius: 8px;">
					<table class=" table table-hover">
						<thead>
							<tr>
								<td colspan="3" style="border:none">Província: <b><?php echo $provincia; ?></b></td>
							</tr>
							<tr>
								<th>Programa </th>
								<th>Projectos </th>
