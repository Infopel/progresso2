    ...
    public function getScriptContr()
    {
        ...
        while ($val = @$GLOBALS['dbi']->fetchRow($alltab_rs)) {
            ...
            if ($row !== false) {
                foreach ($row as $field => $value) {
                    $conn['C_NAME'][$i] = '';
                    $conn['DTN'][$i]    = urlencode($GLOBALS['db'] . "." . $val[0]);
                    $conn['DCN'][$i]    = urlencode($field);
                    $conn['STN'][$i]    = urlencode(
                        $value['foreign_db'] . "." . $value['foreign_table']
                    );
                    $conn['SCN'][$i]    = urlencode($value['foreign_field']);
                    $i++;
                }
            }
            ...
            if ($row !== false) {
                foreach ($row['foreign_keys_data'] as $one_key) {
                    foreach ($one_key['index_list'] as $index => $one_field) {
                        $conn['C_NAME'][$i] = $one_key['constraint'];
                        $conn['DTN'][$i]    = urlencode($GLOBALS['db'] . "." . $val[0]);
                        $conn['DCN'][$i]    = urlencode($one_field);
                        $conn['STN'][$i]    = urlencode(
                            (isset($one_key['ref_db_name']) ?
                                $one_key['ref_db_name'] : $GLOBALS['db'])
                            . "." . $one_key['ref_table_name']
                        );
                        $conn['SCN'][$i] = urlencode($one_key['ref_index_list'][$index]);
                        $i++;
                    }
                }
            }
        ...
        for ($i = 0, $cnt = count($conn["C_NAME"]); $i < $cnt; $i++) {
            ...
            if (in_array($dtn_i, $GLOBALS['designer_url']["TABLE_NAME"])
                && in_array($conn['STN'][$i], $GLOBALS['designer_url']["TABLE_NAME"])
            ) {
                $retval[$ti][$c_name_i][$dtn_i] = array();
                $retval[$ti][$c_name_i][$dtn_i][$conn['DCN'][$i]] = array(
                    0 => $conn['STN'][$i],
                    1 => $conn['SCN'][$i]
                );
            }
