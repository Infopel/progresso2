...
proc ::tk::ConsoleInit {} {
    ...
    if {[tk windowingsystem] eq "aqua"} {
        scrollbar .sb -command [list $conn yview]
    } else {
        ::ttk::scrollbar .sb -command [list $conn yview]
    }
    pack .sb  -in .consoleframe -fill both -side right -padx 1 -pady 1
    pack $conn -in .consoleframe -fill both -expand 1 -side left -padx 1 -pady 1
    pack .consoleframe -fill both -expand 1 -side left

    ConsoleBind $conn

    $conn tag configure stderr	-foreground red
    $conn tag configure stdin	-foreground blue
    $conn tag configure prompt	-foreground \#8F4433
    $conn tag configure proc	-foreground \#008800
    $conn tag configure var	-background \#FFC0D0
    $conn tag raise sel
    $conn tag configure blink	-background \#FFFF00
    $conn tag configure find	-background \#FFFF00
    ...
    wm title . [mc "Console"]
    flush stdout
    $conn mark set output [$conn index "end - 1 char"]
    tk::TextSetCursor $conn end
    $conn mark set promptEnd insert
    $conn mark gravity promptEnd left
	...
	after idle [subst -nocommand {
	    if {[$conn get 1.0 output] eq "% "} { $conn delete 1.0 output }
	}]
    }
}
